...
public class ServerPlaceholders {
    public static void register() {
        Placeholders.register(Identifier.of("server", "tps"), (ctx, arg) -> {
            double tps = TimeUnit.SECONDS.toMillis(1) / Math.max(ctx.server().getAverageTickTime(), ctx.server().method_70562().method_69012().getMillisPerTick());
            String format = "%.1f";
        ...
        Placeholders.register(Identifier.of("server", "online"), (ctx, arg) -> PlaceholderResult.value(String.valueOf(ctx.server().method_70562().method_68990().getCurrentPlayerCount())));
        Placeholders.register(Identifier.of("server", "max_players"), (ctx, arg) -> PlaceholderResult.value(String.valueOf(ctx.server().method_70562().method_68990().getMaxPlayerCount())));
